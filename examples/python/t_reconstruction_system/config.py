...
class ConfigParser(configargparse.ArgParser):
    ...
    def __init__(self):
        super().__init__(
            default_config_files=[os.path.join(os.path.dirname(__file__), "default_config.yml")], conflict_handler="resolve"
        )
        ...
        odometry_parser.add(
            ...
            help="Method used in pose estimation between RGBD images." "Frame2model only available for the tensor engine.",
        )
        odometry_parser.add(
            "--odometry_loop_interval", type=int, help="Intervals to check loop closures between RGBD images."
        )
        odometry_parser.add(
        ...
        registration_parser.add(
            "--icp_voxelsize", type=float, help="Voxel size used to down sample point cloud for fast/multiscale ICP."
        )
        registration_parser.add(
            "--icp_distance_thr", type=float, help="Default distance threshold to filter outliers in ICP correspondences."
        )
        registration_parser.add(
            "--global_registration_method",
            type=str,
            choices=["fgr", "ransac"],
            help="Method used in global registration of two fragment point clouds without an initial pose estimate.",
        )
        registration_parser.add(
            "--registration_loop_weight",
            type=float,
