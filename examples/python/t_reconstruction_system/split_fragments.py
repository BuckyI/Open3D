...
def split_fragments(config):
    ...
    for i in range(0, len(depth_file_names), config.fragment_size):
        ...
        np.savetxt(
            os.path.join(config.path_dataset, "fragments", "fragment_{:03d}_colors.txt".format(frag_id)),
            color_file_names[start:end],
            fmt="%s",
            delimiter="",
        )
        np.savetxt(
            os.path.join(config.path_dataset, "fragments", "fragment_{:03d}_depths.txt".format(frag_id)),
