...
from rgbd_odometry import rgbd_loop_closure, rgbd_odometry
from split_fragments import load_fragments, split_fragments
...
if __name__ == "__main__":
    ...
    if config.fragment_odometry:
        ...
        for frag_id, (depth_list, color_list) in enumerate(zip(depth_lists, color_lists)):
            ...
            edges, trans, infos = rgbd_odometry(depth_list, color_list, intrinsic, config)
            pose_i2w = np.eye(4)
            ...
            edges, trans, infos = rgbd_loop_closure(depth_list, color_list, intrinsic, config)
            ...
            pose_graph.solve_()
            pose_graph.save(os.path.join(config.path_dataset, "fragments", "fragment_posegraph_{:03d}.json".format(frag_id)))
        end = time.time()
    ...
    if config.fragment_integration:
        ...
        for frag_id, (depth_list, color_list) in enumerate(zip(depth_lists, color_lists)):
            pose_graph = PoseGraphWrapper.load(
                os.path.join(config.path_dataset, "fragments", "fragment_posegraph_{:03d}.json".format(frag_id))
            )
            extrinsics = pose_graph.export_extrinsics()

            vbg = integrate(depth_list, color_list, intrinsic, intrinsic, extrinsics, config=config)

            pcd = vbg.extract_point_cloud()

            # Float color does not load correctly in the t.io mode
            o3d.io.write_point_cloud(
                os.path.join(config.path_dataset, "fragments", "fragment_pcd_{:03d}.ply".format(frag_id)), pcd.to_legacy()
            )
