...
class ReconstructionWindow:
    ...
    def _on_start(self):
        ...
        self.model = o3d.t.pipelines.slam.Model(
            self.voxel_size_slider.double_value,
            16,
            self.est_block_count_slider.int_value,
            o3c.Tensor(np.eye(4)),
        ...
        if self.is_scene_updated:
            if pcd is not None and pcd.point.positions.shape[0] > 0:
                self.widget3d.scene.scene.update_geometry(
                    "points", pcd, rendering.Scene.UPDATE_POINTS_FLAG | rendering.Scene.UPDATE_COLORS_FLAG
                )
    ...
    def update_main(self):
        ...
        while not self.is_done:
            ...
            if (
                ...
                pcd = self.model.voxel_grid.extract_point_cloud(3.0, self.est_point_count_slider.int_value).to(
            ...
            info += "Transformation:\n{}\n".format(
                np.array2string(T_frame_to_model.numpy(), precision=3, max_line_width=40, suppress_small=True)
            )
            info += "Active voxel blocks: {}/{}\n".format(
                self.model.voxel_grid.hashmap().size(), self.model.voxel_grid.hashmap().capacity()
            )
            info += "Surface points: {}/{}\n".format(
                0 if pcd is None else pcd.point.positions.shape[0], self.est_point_count_slider.int_value
            )
